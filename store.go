...
func (cs *CascadeStore) setInRequest(r *http.Request, session *sessions.Session, key string, serialized []byte) (err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			err := r.(error)
			store_log.Errorf(ctx, err, "Could not save session in request: %s", err)
		}
	}()
...
func (cs *CascadeStore) setInMemcache(r *http.Request, session *sessions.Session, key string, serialized []byte) (err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			err := r.(error)
			store_log.Errorf(ctx, err, "Could not save session in Memcache: %s", err)
		}
	}()
...
func (cs *CascadeStore) save(r *http.Request, session *sessions.Session) (err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			err := r.(error)
			store_log.Errorf(ctx, err, "Could not save session: %s", err)
		}
	}()
...
func (cs *CascadeStore) load(r *http.Request, session *sessions.Session) (success bool, err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			err := r.(error)
			store_log.Errorf(ctx, err, "Could not load session: %s", err)
		}
	}()
...
func (cs *CascadeStore) delete(r *http.Request, session *sessions.Session) (err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			err := r.(error)
			store_log.Errorf(ctx, err, "Could not delete session: %s", err)
		}
	}()
